...
# Επικεφαλίδα για το αρχείο αποτελεσμάτων
...
for n in "${DEGREES[@]}"; do
    for p in "${PROCS[@]}"; do
        ...
        # Εκτέλεση και απομόνωση του χρόνου (iv)
        # Ψάχνουμε τη γραμμή "(iv)" και παίρνουμε την 5η στήλη (τον αριθμό)
...
for spar in "${SPARSITY_VALS[@]}"; do
    for p in "${PROCS[@]}"; do
        ...
        # Απομόνωση χρόνων CSR (iv) και Dense (v)
